...
/**
 * This is the model implementation of QuandlEURONEXT.
 * 
 *
 * @author Heng Chi
 */
public class QuandlEuronext__NodeModel extends NodeModel {

	// the logger instance
	private static final NodeLogger logger = NodeLogger.getLogger(QuandlEuronext__NodeModel.class);

	public static QuandlEuronext__Config m_config = new QuandlEuronext__Config();

	public static ArrayList<String> m_tickers = new ArrayList<String>();

	protected QuandlEuronext__NodeModel() {
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData, final ExecutionContext exec)
		...
		if (m_tickers.size() == 0) {
			...
			container = exec.createDataContainer(createEURONEXTDataColumnSpec());

			Long count = 0L;

			for (String ticker : m_tickers) {

				String urlStr = "https://www.quandl.com/api/v3/datasets/EURONEXT/" + ticker + ".json?"
	...
	protected void loadValidatedSettingsFrom(final NodeSettingsRO settings) throws InvalidSettingsException {
		...
		System.out.println("QuandlEURONEXTNodeModel: Enter into loadValidatedSettingsFrom()");
		QuandlEuronext__NodeModel.m_config.loadSettingsInModel(settings);
		System.out.println("m_tickers.size() = " + m_tickers.size());
	...
	protected void validateSettings(final NodeSettingsRO settings) throws InvalidSettingsException {
		...
		System.out.println("QuandlEURONEXTNodeModel: Enter into validateSettings()");
		QuandlEuronext__NodeModel.m_config.loadSettingsInModel(settings);
		System.out.println("m_tickers.size() = " + m_tickers.size());
		System.out.println("m_tickers.getm_api_key() = " + m_config.getm_api_key() + "m_tickers.getM_start_date() = "
				+ m_config.getM_start_date() + "m_tickers.getM_end_date() = " + m_config.getM_end_date());
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs) throws InvalidSettingsException {
		...
		if (m_config == null) {
			throw new InvalidSettingsException("QuandlEURONEXTNodeModel: No settings available");
		}

		return new DataTableSpec[] { createEURONEXTDataColumnSpec() };
	}

	private DataTableSpec createEURONEXTDataColumnSpec() {
		...
		allColSpecs[5] = new DataColumnSpecCreator("Last", DoubleCell.TYPE).createSpec();
		allColSpecs[6] = new DataColumnSpecCreator("Volume", DoubleCell.TYPE).createSpec();
		allColSpecs[7] = new DataColumnSpecCreator("Turnover", DoubleCell.TYPE).createSpec();
	...
	protected void loadInternals(final File internDir, final ExecutionMonitor exec)
		...
		System.out.println("QuandlEURONEXTNodeModel: Enter into loadInternals()");
	}
	...
	protected void saveInternals(final File internDir, final ExecutionMonitor exec)
		...
		System.out.println("QuandlEURONEXTNodeModel: Enter into saveInternals()");
	...
	public static JsonObject getJSONObjectFromURL(String urlStr) {
		...
		try {
			...
			if (urlConnection instanceof HttpURLConnection) {
				...
				System.out.println("QuandlEURONEXTNodeModel: No connection");
				return null;
			}
